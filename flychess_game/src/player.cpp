...
namespace flychess_game {
    void ChessPiece::preGoalMove(int steps, int pre_goal_position) {
        ...
        if(!piece_info.if_pre_goal) {
            int new_position = piece_info.position + steps;// >=pre_goal_position
            new_position -= pre_goal_position; // 
            if (new_position == 0) {
                piece_info.position = pre_goal_position; // 0pre_goal_position
            }
            else {
            ...
            if (piece_info.position == pre_goal_position) {
                ...
                if (1) {
                    ...
                    if (new_position == 58){
                        ...
                        new_position -= 58; // 58
                        // pregoalpre_goal_position
                        piece_info.position = 58 - new_position; // pre_goal_position + 
                    ...
                    if (new_position < 52) {
                        ...
                        piece_info.position = pre_goal_position; // 52pre_goal_position
                        forward = true;
                        return;
                    }
                    else {
    ...
    Player::Player(game_utils::Color color, int chess_piece_count){
        ...
        switch(static_cast<int>(color)){
            case 0:
                ...
                start_position = 1;
                index_fly = 2;
                bridge_start_end_position = {18, 30}; // 
                break;
            case 1:
                pregoal_position = 37;
                start_position = 40;
